...
- **PDF rendering reuses the Phase 5 print-agent technique, not a new dependency** — no PDF library existed in the project; `playwright-core` already did (Phase 5, for receipt/kitchen-ticket screenshotting, because ESC/POS printers can't shape Persian text themselves). `src/lib/pdf-render.ts` mirrors `print-agent/render.ts`'s approach almost exactly (launch Chromium, embed the Vazirmatn font as a data URI, render to a buffer) but runs inside this app's own Next.js server rather than the standalone till-PC print agent, so it resolves the font path from `process.cwd()` instead of `__dirname` (reliable once Next bundles the route handler; `__dirname` inside a bundled handler isn't).
...
| A user can build a new custom report (source + metric + dimension + filter), no code | `/dashboard/reports` → «گزارش‌ساز» (`report-builder-section.tsx`), backed by `GET /api/reports/views` (the whitelist) and `POST /api/reports/query` |
