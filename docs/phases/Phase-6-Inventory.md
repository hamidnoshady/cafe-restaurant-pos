...
| Low-stock alert fires on crossing the reorder threshold | `crossedLowStockThreshold` (`src/lib/inventory.ts`) checked inside `consumeInventory`; broadcasts `inventory.low_stock` (`src/lib/realtime.ts`) live, and `GET /api/inventory/low-stock` for the dashboard's banner (`/accounting/inventory`) |

Dashboard UI: `/accounting/inventory` (`src/app/dashboard/inventory/inventory-manager.tsx`) — tabs for items, recipes (menu item + modifier), suppliers, purchasing, waste, and stock counts, plus a live low-stock banner.
