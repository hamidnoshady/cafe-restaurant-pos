...
5. **Manual journal entry permissions** — Owner/Manager only, matching every other back-office/financial surface (`/dashboard/ledger`, same as `/accounting/inventory`'s admin tabs — see Phase 6 decision 6).
...
Dashboard UI: `/dashboard/ledger` (`src/app/dashboard/ledger/ledger-manager.tsx`) — tabs for trial balance, journal (all auto-posted + manual entries with their lines), and manual entry. Purchasing UI (`/accounting/inventory` → «خرید») gained a settlement-method picker (نقدی/بانک/نسیه) shown at receive time.
