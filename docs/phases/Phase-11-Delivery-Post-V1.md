...
| Delivery reports alongside standard reports | Views `v_delivery_performance`, `v_courier_performance` (`migrations/0011_delivery.sql`); whitelisted in `REPORT_VIEWS` and added to `STANDARD_REPORTS` (`src/lib/reports.ts`), so they render in `/dashboard/reports` and are pinnable to the dashboard like any standard report. |
