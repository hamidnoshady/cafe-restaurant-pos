...
4. **Number of kitchen stations** — **one shared KDS screen** (`/dashboard/kitchen`). `printer_kind` already distinguishes `receipt`/`kitchen` for Phase 5 printing, but v1 doesn't split tickets by station — every kitchen-role login sees the same queue.
