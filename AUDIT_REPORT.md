...
| `17439d3` | 8 | `electron/app-paths.js` (new — Configuration/Data/Backup/Logs split + upgrade migration), `electron/backend-manager.js` (derive paths from `computePaths()`; independent `PG_TOOLS_DIR`/`RESTORE_EMERGENCY_DIR` env-passthrough bug fix), `electron/certificate-manager.js`, `electron/logger.js` (same), `electron/main.js` (runs the migration once; fixes the `hasExistingConfigAtDefault` interaction bug; fixes a stray flat-path smoke-test reference), `electron/package.json` (packaging list), `src/lib/app-paths.test.ts` (new, 10 tests), `src/lib/backend-manager-environment.test.ts` (new, 4 tests), `docs/standalone-desktop-app.md`, `AUDIT_REPORT.md`/`TESTING_CHECKLIST.md` |
| `372c629` | 8 (CI fixes) | `electron/main.js` (missed `computePaths` import — crashed every packaged launch), `electron/app-paths.js` (new `isCaseOnlyRename()`; the `logs` → `Logs` conflict check and `moveEntry()` both fixed for NTFS case-insensitivity), `src/lib/app-paths.test.ts` (new Windows/NTFS regression suite using an in-memory case-insensitive fake filesystem, 13 tests total), `AUDIT_REPORT.md`/`TESTING_CHECKLIST.md` (documents both regressions and why CI, not the sandbox, caught them) |
