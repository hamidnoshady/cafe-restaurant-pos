...
export default function PlatformMessagingPage() {
  ...
  return <div className="mx-auto w-full max-w-6xl space-y-4 sm:space-y-6">
    <header>
      ...
      <p className="mt-1 text-sm text-white/50">کلیدهای پلتفرم، نرخ مصرف و درخواست‌های شارژ کسب‌وکارها. کلیدها هرگز دوباره نمایش داده نمی‌شوند.</p>
    </header>
    ...
    <Card title="ارائه‌دهنده و نرخ">
      <form onSubmit={saveConfig} className="grid gap-3 sm:grid-cols-2">
        ...
        <Field label="هزینه هر قطعه پیامک (ریال)"><input className={inputClass} type="number" min="0" value={draft.rate.smsRialPerSegment} disabled={!canManage} onChange={(e) => setDraft({ ...draft, rate: { ...draft.rate, smsRialPerSegment: Number(e.target.value) } })} /></Field>
        ...
        <Field label="هزینه هر ایمیل (ریال)"><input className={inputClass} type="number" min="0" value={draft.rate.emailRialPerSend} disabled={!canManage} onChange={(e) => setDraft({ ...draft, rate: { ...draft.rate, emailRialPerSend: Number(e.target.value) } })} /></Field>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card title="بسته‌های اعتبار">
        <div className="space-y-2">{data.packages.length ? data.packages.map((pkg) => <div className="flex items-center justify-between rounded-lg border border-white/10 px-3 py-2 text-sm" key={pkg.id}><span>{pkg.name}{!pkg.isActive ? " · غیرفعال" : ""}</span><span>{fmt(pkg.creditAmountRial)} اعتبار · {fmt(pkg.priceRial)}</span></div>) : <p className="text-sm text-white/50">هنوز بسته‌ای ساخته نشده است.</p>}</div>
        {canManage ? <form onSubmit={addPackage} className="mt-4 grid gap-2 sm:grid-cols-3"><input className={inputClass} placeholder="نام بسته" value={packageName} onChange={(e) => setPackageName(e.target.value)} required /><input className={inputClass} type="number" min="1" placeholder="قیمت ریال" value={packagePrice} onChange={(e) => setPackagePrice(e.target.value)} required /><div className="flex gap-2"><input className={inputClass} type="number" min="1" placeholder="اعتبار ریال" value={packageCredit} onChange={(e) => setPackageCredit(e.target.value)} required /><Button type="submit" disabled={busy === "package"}>افزودن</Button></div></form> : null}
      </Card>
      <Card title="درخواست‌های شارژ">
        <div className="space-y-2">{data.requests.length ? data.requests.map((request) => <div key={request.id} className="rounded-lg border border-white/10 px-3 py-2 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><span>{request.businessName ?? request.businessId} · {request.packageName}</span><b>{request.status === "pending" ? "در انتظار" : request.status === "approved" ? "تأیید شد" : "رد شد"}</b></div><p className="mt-1 text-xs text-white/50">{fmt(request.creditAmountRial)} · {date(request.createdAt)}</p>{canManage && request.status === "pending" ? <div className="mt-2 flex gap-2"><Button className="h-8" onClick={() => void submit({ action: "review_top_up", requestId: request.id, status: "approved" }, `approve:${request.id}`)} disabled={Boolean(busy)}>تأیید</Button><Button className="h-8" variant="danger" onClick={() => void submit({ action: "review_top_up", requestId: request.id, status: "rejected" }, `reject:${request.id}`)} disabled={Boolean(busy)}>رد</Button></div> : null}</div>) : <p className="text-sm text-white/50">درخواستی وجود ندارد.</p>}</div>
      </Card>
    </div>
  </div>;
}
