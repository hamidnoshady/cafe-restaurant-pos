...
describe("the Accounting menu is written for RTL", () => {
  ...
  it("ties a group's children to their heading with a start-side rule", () => {
    expect(NAV_SOURCE).toMatch(/border-s\b/);
    expect(NAV_SOURCE).toMatch(/\bms-4\b/);
  });
  ...
  it("flips the disclosure chevron for RTL", () => {
    ...
    expect(NAV_SOURCE).toMatch(/-rotate-90 rtl:rotate-90/);
  ...
  it("labels every entry in text, never by glyph alone", () => {
    ...
    expect(NAV_SOURCE).toMatch(/tooltip=\{entry\.label\}/);
    expect(NAV_SOURCE).toMatch(/\{entry\.label\}/);
  });

  it("makes each group a real disclosure a screen reader can follow", () => {
    expect(NAV_SOURCE).toMatch(/aria-expanded=\{open\}/);
    expect(NAV_SOURCE).toMatch(/aria-controls=\{panelId\}/);
  ...
  it("marks the current page for assistive tech, not only in colour", () => {
    expect(NAV_SOURCE).toMatch(/aria-current=\{active \? "page" : undefined\}/);
  });

  it("keeps a visible focus ring on the group toggle", () => {
    expect(NAV_SOURCE).toMatch(/focus-visible:ring-2/);
    // Paired light/dark, the way the design system requires.
    expect(NAV_SOURCE).toMatch(/dark:focus-visible:ring-/);
  });

  it("hides labels, not entries, when the rail collapses to icons", () => {
    expect(NAV_SOURCE).toMatch(/group-data-\[state=collapsed\]\/sidebar:hidden/);
  });
});
