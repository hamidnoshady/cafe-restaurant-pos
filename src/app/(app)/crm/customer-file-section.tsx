...
export function CustomerFileSection({ customerId, role }: { customerId: string; role: string }) {
  ...
  const loadTimeline = useCallback(() => {
    ...
    api<{ events: TimelineEvent[] }>(`/api/crm/customers/${customerId}/timeline${query}`).then(
      ({ ok, data }) => {
  ...
  const removeNote = async (noteId: string) => {
    const { ok } = await api(`/api/crm/customers/${customerId}/notes?noteId=${noteId}`, {
      method: "DELETE",
    });
    if (ok) loadFile();
  };
  ...
  const pinNote = async (note: Note) => {
    const { ok } = await api(`/api/crm/customers/${customerId}/notes`, {
      method: "PATCH",
    ...
    <div className="min-w-0 space-y-4">
      ...
      >
        ...
        {stage ? (
          ...
          <p className="mt-3 text-xs leading-6 text-muted-foreground">
            هنوز امتیاز چرخهٔ عمر محاسبه نشده است. از میز کار، «محاسبهٔ دوباره» را بزنید.
          </p>
        )}
        ...
        actions={
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              {(Object.keys(TIMELINE_KIND_LABELS) as TimelineKind[]).map((kind) => (
