 ...
 * an unchanged value can omit the key — the service keeps what is stored.
 */
function accountingFieldsIn(body: Record<string, unknown>): { field: string } | null {
  ...
  if (general) {
    // The rate is the ledger's number; the two identity numbers are what the ledger
    // matches an invoice and a tax return against, so they are treated the same way
    // — and the collection route gates them identically, or a create could write what
    // an edit refuses to change.
    for (const key of ["taxPercentage", "nationalId", "economicCode"]) {
      if (general[key] !== undefined) return { field: `general_info.${key}` };
    }
  }
