 ...
 * already used, reused verbatim for background removal: refuse before the
 * provider call when the wallet plainly cannot cover the price, charge only
 * after bytes are in hand, and store the result as a new `bg_removed`
...
vi.mock("@/lib/ai-media-service", async (importOriginal) => {
  ...
  return { ...actual, runMediaBackgroundRemoval: vi.fn() };
});
...
function req(): NextRequest {
  return new Request(`http://localhost:3000/api/media/${ASSET_ID}/bg-remove`, { method: "POST" }) as unknown as NextRequest;
}
...
beforeEach(() => {
  ...
  vi.mocked(mediaService.storeMediaAsset).mockResolvedValue({ id: "new-asset", variant: "bg_removed" } as never);
});

describe("POST /api/media/[id]/bg-remove", () => {
  ...
  it("402s before ever calling the provider when the wallet cannot cover the price", async () => {
    ...
    expect(aiMediaService.runMediaBackgroundRemoval).not.toHaveBeenCalled();
  });

  it("stores the result as a new bg_removed asset and charges the wallet only after success", async () => {
    vi.mocked(aiMediaService.runMediaBackgroundRemoval).mockResolvedValue({ bytes: Buffer.from("png-bytes"), costUsd: 0.01 });
    const res = await POST(req(), ctx());
    expect(res.status).toBe(201);

    const [storeArgs] = vi.mocked(mediaService.storeMediaAsset).mock.calls[0];
    expect(storeArgs).toMatchObject({ variant: "bg_removed", sourceAssetId: ASSET_ID, mimeType: "image/png" });

    expect(walletService.chargeFeatureUse).toHaveBeenCalledWith(
      expect.objectContaining({ featureKey: "media_bg_remove", priceRial: 5000 }),
    );
  });
  ...
  it("maps a provider auth failure to 502 without ever charging the wallet", async () => {
    ...
    vi.mocked(aiMediaService.runMediaBackgroundRemoval).mockRejectedValue(new MediaAiError("ai_auth", "کلید نامعتبر است."));
    const res = await POST(req(), ctx());
