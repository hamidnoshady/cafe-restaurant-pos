...
describe("DELETE /api/media/[id]", () => {
  it("answers 409 asset_in_use with the usage payload when a catalogue item still shows it", async () => {
    vi.mocked(mediaService.getMediaAssetUsage).mockResolvedValue({ menuItems: [{ id: "m1", name: "قهوه" }], inventoryItems: [] } as never);
  ...
  it("plain delete trashes an unused asset", async () => {
    vi.mocked(mediaService.getMediaAssetUsage).mockResolvedValue({ menuItems: [], inventoryItems: [] } as never);
