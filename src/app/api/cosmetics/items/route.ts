...
import { listCosmeticBoard } from "@/lib/cosmetics-service";
import { validateVariantAttributes, type VariantAttributeInput } from "@/lib/items";

/** The cosmetics board: every product family and its variants, with attributes, stock and pricing. */
export const GET = withTenantScope(async () => {
  ...
  const items = await listCosmeticBoard(location.id);
  return NextResponse.json({ items });
});

/**
 * Creates either a product family (no parentItemId) or one variant of one
 * (parentItemId + its distinguishing attributes) — the same two shapes the
 * `items` model already distinguishes for accessories.
 */
