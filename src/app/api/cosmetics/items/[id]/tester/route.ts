...
import { openTester } from "@/lib/cosmetics-service";
...
/** Opens one sellable unit as a تستر — its cost moves to «کالای منقضی و تستر». */
export const POST = withTenantScope(async (_request: NextRequest, context: { params: Promise<{ id: string }> }) => {
  ...
  try {
    ...
    const result = await openTester(client, {
    ...
    return NextResponse.json({ error: "tester_failed", message: (err as Error).message }, { status: 400 });
  } finally {
    client.release();
  }
});
