    ...
    >
      ...
      <div className="flex items-center gap-2.5">
        ...
        >
          ...
          <span
            className="min-w-7 text-center text-sm font-bold text-[#252522]"
            aria-live="polite"
          >
        ...
        <p
          className="min-w-0 flex-1 truncate font-bold text-[#252522]"
          title={line.name}
        >
          {line.name}
        </p>
      ...
      {hasAddOns ? (
        <div className="me-1 mt-2 rounded-lg border border-[#F2D097]/70 bg-[#FFF9EE] px-2.5 py-2">
          <ModifierBadges modifiers={line.modifiers} tone="amber" showCaption />
          <p className="mt-1.5 flex flex-wrap items-center gap-x-1 text-[11px] text-[#77756F]">
            ...
            <span className="text-[#77756F]">برای هر واحد</span>
          </p>
        </div>
      ) : (
        <p className="me-1 mt-1.5 text-xs text-[#77756F]">
          {money.format(line.unitPrice)} برای هر واحد
        </p>
      )}

      {line.note ? (
        <p className="me-1 mt-2 text-xs text-[#77756F]">
          یادداشت: {line.note}
        </p>
      ) : null}
      ...
      <div className="mt-2 flex items-center justify-start gap-1 border-t border-[#EAE8E2]/70 pt-2">
        <button
          type="button"
          onClick={() => onEdit(line.key)}
          className="flex min-h-9 items-center gap-1.5 rounded-lg px-2.5 text-xs font-bold text-[#9B6700] transition-colors hover:bg-[#FFF1D8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E9A11B]/45"
        >
