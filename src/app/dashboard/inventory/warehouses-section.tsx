...
/**
 * Phase 42 — «لیست انبارها» (with the «افزودن انبار» form).
 *
 * A warehouse is a branch: the table below is the business's locations with
 * what each one holds — item count, stock value (the canonical
 * v_inventory_valuation figure), low-stock count and the last movement — and
 * the form on top adds a new one through the same createBranch service the
 * branches screen uses.
 */
...
export function WarehousesSection({ onOpenStock }: { onOpenStock: (locationId: string) => void }) {
  const money = useMoney();
  const [warehouses, setWarehouses] = useState<Warehouse[] | null>(null);
  const [error, setError] = useState("");

  const load = useCallback(() => {
    api<WarehousesResponse>("/api/inventory/warehouses").then(({ ok, data }) => {
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <AddWarehouseForm onCreated={load} />
      <SectionCard
        title={
          <div>
        ...
        description="هر انبار یک شعبه است؛ موجودی، تأمین‌کنندگان و سندهای انبار به هر انبار تعلق دارند."
        flush
      >
        {warehouses === null ? (
          ...
          <div className="p-4 sm:p-5">
            <EmptyState>هنوز انباری ثبت نشده است. با فرم بالای صفحه اولین انبار را بسازید.</EmptyState>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[720px] text-sm">
              <thead>
                <tr className="border-b border-border bg-stone-50 dark:bg-stone-900/40">
                  ...
                  <th className="px-4 py-3 text-start text-xs font-medium text-stone-500 dark:text-stone-400 sm:text-sm">آخرین حرکت</th>
...
function AddWarehouseForm({ onCreated }: { onCreated: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    setError("");
    const { ok, data } = await api("/api/inventory/warehouses", {
      method: "POST",
  ...
  return (
    <SectionCard
      ...
      description="نام انبار را بنویسید؛ آدرس و تلفن اختیاری است."
    >
