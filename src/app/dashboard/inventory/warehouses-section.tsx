...
/**
 * Phase 42 — «لیست انبارها».
 *
 * Warehouses are the business's locations. This view deliberately owns the
 * complete list experience instead of relying on the branch settings page:
...
export function WarehousesSection({ onOpenStock }: { onOpenStock: (locationId: string) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const result = await api<WarehousesResponse>("/api/inventory/warehouses");
      if (!result.ok) {
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <ErrorBox>{error && warehouses !== null ? error : ""}</ErrorBox>
      <SectionCard
        title={
          <div>
        ...
        description="هر انبار یک شعبه است؛ موجودی و سندهای انبار به آن تعلق دارند."
        actions={
...
function AddWarehouseForm({ onCreated }: { onCreated: () => void | Promise<void> }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setDone("");
    try {
      const result = await api("/api/inventory/warehouses", {
        method: "POST",
