...
/**
 * Tables with no tenant data — the single canonical list. Both this module's
 * export/restore path and `integration/tenant-isolation.integration.test.ts`'s
 * RLS coverage check import this Set rather than keeping their own copy; the
 * two drifted apart once before (see migration 0130's addition of the billing
 * tables and Phase 24's login/MFA/rate-limit tables, which only ever landed in
 * the test's copy), which is exactly the failure mode a single source fixes.
 */
export const EXEMPT_TABLES = new Set([
  ...
  // Phase 18 & Phase 37 & Phase 39 — singleton platform provider config
  // (platform_ai_gateway) plus globally shared priced catalogues.
  "platform_ai_gateway",
  "ai_credit_packages",
