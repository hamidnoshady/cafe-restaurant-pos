...
describe("the probe", () => {
  ...
  it("never asks the proxy for models without an admin key", async () => {
    const urls = stubProxy({
      "/health/liveliness": { status: 200, body: "I'm alive!" },
    });
  ...
  it("treats a missing model listing as healthy, not as a fault", async () => {
    // An older proxy build answers 404 for /model/info; the proxy is still
    // alive and serving, so the probe must not call it a failure.
    stubProxy({
      "/health/liveliness": { status: 200, body: "I'm alive!" },
    });
