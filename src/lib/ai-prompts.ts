...
export const PROMPT_FRAGMENTS: Record<FragmentKey, string> = {
  ...
  "app:website":
    "مدیریت وب‌سایت: می‌توانی مطلب و محصول برای سایت پیش‌نویس کنی (website.post.draft، website.post.update، website.product.upsert) و وضعیت اتصال و صف ارسال را ببینی (get_website_status، list_website_posts، list_website_products). قاعده: سایت ویترین است و منبع حقیقتِ قیمت و موجودی همین نرم‌افزار است — در پیش‌نویس فقط از داده‌های واقعی استفاده کن: نام و قیمت آیتم‌ها را با find_items یا get_menu_item_details بخوان و هیچ عدد، قیمت یا ادعایی از خودت نساز. هر پیش‌نویس پیش‌نویس می‌ماند؛ انتشار (website.post.publish) همیشه با تأیید صریح کاربر است و حتی با اجرای خودکار باز هم به تأیید نیاز دارد. اگر وب‌سایتی متصل نیست، بگو از «اتصال‌ها ← وب‌سایت» می‌توان متصل کرد.",
