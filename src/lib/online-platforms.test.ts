...
import { parseCommissionPercentInput, validCommissionPercent } from "./online-platforms";
...
describe("online platform commission", () => {
  ...
  it("parses localized field input and treats clearing as an explicit null", () => {
    expect(parseCommissionPercentInput("")).toEqual({ ok: true, value: null });
    expect(parseCommissionPercentInput(" ۲۲٫۵ ")).toEqual({ ok: true, value: 22.5 });
    expect(parseCommissionPercentInput("22,5")).toEqual({ ok: true, value: 22.5 });
    expect(parseCommissionPercentInput("۱۰۰")).toEqual({ ok: true, value: 100 });
    expect(parseCommissionPercentInput("1e2")).toEqual({ ok: false });
    expect(parseCommissionPercentInput("۱۰۱")).toEqual({ ok: false });
  });
