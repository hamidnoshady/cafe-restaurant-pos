 ...
 * money only moves after a claim, and a failed wallet charge rolls the claim
 * back — the website billing tick's proven pattern, applied to the platform
 * plan itself.
 *
...
const SUBSCRIPTION_COLUMNS = `s.business_id, b.name AS business_name, s.plan_key, s.status, s.started_at,
       s.current_period_start, s.current_period_end, s.trial_end, s.grace_end,
...
export async function changeBusinessPlan(input: {
  ...
  if (existing && existing.planKey === input.planKey && existing.status !== "expired" && existing.status !== "cancelled") {
    return { subscription: existing, plan, outcome: "unchanged" };
  }
  if (plan.status === "retired") throw new SubscriptionError("plan_retired");
  ...
  try {
    ...
    // Downgrade vs upgrade: a paid period already running keeps its end when
    // the plan changes mid-period (the console's manual assignment has never
    // re-charged mid-month, and that behaviour is preserved). A new
    // subscription (or one reviving from expired/cancelled) starts a fresh
    // period.
    const keepPeriod =
      current &&
    ...
    await client.query(
      `INSERT INTO business_subscriptions
         (business_id, plan_key, status, started_at, current_period_start, current_period_end,
          trial_end, grace_end, cancel_at_period_end, auto_renew, cancelled_at)
       VALUES ($1, $2, 'active', $3, $4, $5, $6, NULL, false, $7, NULL)
       ON CONFLICT (business_id) DO UPDATE SET
         plan_key = EXCLUDED.plan_key,
         status = 'active',
         current_period_start = CASE WHEN $8 THEN business_subscriptions.current_period_start
                                     ELSE EXCLUDED.current_period_start END,
         current_period_end = CASE WHEN $8 THEN business_subscriptions.current_period_end
      ...
      [
        ...
        plan.trialDays > 0 ? addDays(periodStart, plan.trialDays) : null,
...
/** Sequential, human-readable invoice numbers: INV-<year><month>-<seq per month>. */
async function nextInvoiceNumber(client: PoolClient): Promise<string> {
  const { rows } = await client.query<{ prefix: string; count: string }>(
    `SELECT to_char(now(), 'YYYYMM') AS prefix,
            (SELECT count(*)::text FROM billing_invoices
              WHERE created_at >= date_trunc('month', now())) AS count`,
  );
 ...
 * — never a read-then-write. The invoice is claimed before any money moves;
 * a wallet that cannot cover the charge marks the subscription past_due (with
 * the plan's grace window) and the next tick retries. The claimed-but-unpaid
 * invoice stays open as the outstanding amount.
 */
export async function renewBusinessSubscription(
  ...
  try {
    ...
    if (!invoiceId) {
      // Already claimed: this period's renewal ran before (a retried tick).
      await client.query("COMMIT");
      const existing = await findInvoiceByReference(businessId, reference);
      if (existing?.status === "paid") {
        await advancePeriod(businessId, subscription, now);
      }
      return { status: "duplicate", invoiceId: existing?.id ?? null };
    }

    // Wallet debit — inside THIS transaction, so the invoice claim, the debit
    // and the period advance commit or roll back as one unit. Ledger kind
    // 'subscription', the invoice linked in metadata.
    ...
    if (err instanceof WalletInsufficientFundsError) {
      // The claim rolled back with the transaction: the open invoice from a
      // previous failed attempt (if any) remains the outstanding amount, and
      // the next tick retries cleanly. Mark past_due within the grace window.
      await query(
        `UPDATE business_subscriptions
...
async function findInvoiceByReference(businessId: string, reference: string): Promise<InvoiceRecord | null> {
  const { rows } = await query<Record<string, unknown>>(
    `SELECT i.*, b.name AS business_name
       FROM billing_invoices i JOIN businesses b ON b.id = i.business_id
...
export async function runSubscriptionRenewalTick(now: Date = new Date()): Promise<{
  ...
  return { checked: due.length, renewed, pastDue, expired };
}
