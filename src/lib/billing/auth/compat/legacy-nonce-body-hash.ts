 ...
 * with a per-key nonce table on the receiver. New platform code must verify v1 only;
 * this module exists for cutover tests until central Billing v1 is live everywhere.
 */
...
export function signLegacyUsageBody(args: {
  body: string;
  nonce: string;
  secret: string;
  timestampMs: string;
...
export function verifyLegacyUsageSignature(args: {
  body: string;
  nonce: string | null;
  now?: number;
  secret: string;
