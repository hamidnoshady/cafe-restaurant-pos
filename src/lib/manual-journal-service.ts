...
async function attachLines(businessId: string, drafts: DraftRow[]): Promise<JournalDraft[]> {
  if (drafts.length === 0) return [];
  const { rows: lines } = await query<DraftLineRow>(
...
export async function listDrafts(businessId: string): Promise<JournalDraft[]> {
  const { rows } = await query<DraftRow>(
    `SELECT d.id, d.entry_date::text AS entry_date, d.memo, d.created_by, u.full_name AS created_by_name, d.created_at::text AS created_at
...
export async function getDraft(businessId: string, id: string): Promise<JournalDraft | null> {
  const { rows } = await query<DraftRow>(
    `SELECT d.id, d.entry_date::text AS entry_date, d.memo, d.created_by, u.full_name AS created_by_name, d.created_at::text AS created_at
       FROM journal_entry_drafts d LEFT JOIN users u ON u.id = d.created_by
...
export async function createDraft(params: {
  ...
  if (!params.memo.trim()) throw new ManualJournalError("memo_required");
  const nonZero = validatedNonZeroLines(params.lines);
  await assertAccountsOwned(params.businessId, nonZero.map((l) => l.accountId));
  ...
  try {
    ...
    const { rows } = await client.query<{ id: string }>(
      ...
      [params.businessId, params.locationId, params.entryDate?.trim() || null, params.memo.trim(), params.createdBy],
    );
...
export async function deleteDraft(businessId: string, id: string): Promise<void> {
  const { rowCount } = await query(`DELETE FROM journal_entry_drafts WHERE id = $1 AND business_id = $2`, [id, businessId]);
...
export async function approveDraft(params: {
  ...
  const draft = await getDraft(params.businessId, params.draftId);
  ...
  try {
    ...
    await client.query(`DELETE FROM journal_entry_drafts WHERE id = $1`, [params.draftId]);
    await client.query("COMMIT");
...
export async function reverseEntry(params: {
  ...
  const { rows: entryRows } = await query<{
    ...
    `SELECT id, source_type, memo, reverses_entry_id, reversed_at::text AS reversed_at
       FROM journal_entries WHERE id = $1 AND business_id = $2`,
  ...
  const { rows: lineRows } = await query<{ account_id: string; debit: string; credit: string }>(
  ...
  try {
    ...
    const entryId = await postJournalEntry(client, {
      ...
      memo: params.memo?.trim() || `برگشت سند: ${original.memo ?? ""}`.trim(),
      sourceType: "manual",
      sourceId: null,
      createdBy: params.actorId,
    ...
    await client.query(`UPDATE journal_entries SET reversed_at = now(), reversed_by = $2 WHERE id = $1`, [
      params.entryId,
      params.actorId,
    ]);
    await client.query("COMMIT");
